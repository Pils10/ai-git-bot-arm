...
> **Status:** M1 – M3 ✅ shipped  M4 (wave 1 + wave 2 iterations 1 – 4) ✅ shipped  M5 ✅ shipped  M6 ✅ shipped  M7 ✅ shipped  
...
| **M7** — Suite promotion workflow | ✅ shipped | `SuitePromotionService` (`OFFER_AS_PR` / `PROMOTE_ON_MERGE` / `COMMIT_TO_PR` + `EPHEMERAL`), follow-up PR on `ai-tests/pr-{n}` or `ai-tests/promoted-pr-{n}`, conflict-resolving file naming (`_2`, `_3`, …), idempotency via `PrWorkflowRun.followUpPrNumber`, lifecycle-aware teardown in `E2eTestPrCloseHandler`, and a nightly `PromotedSuiteGarbageCollector` (configurable `prworkflow.e2e.promotion.retention`, default 30 days) that retires stale generated suites while keeping the promoted-PR link. User story: [`SUITE_PROMOTION_USER_STORY.md`](./SUITE_PROMOTION_USER_STORY.md); operator recipe: [`doc/PR_WORKFLOWS_E2E.md` § Suite lifecycle modes](../PR_WORKFLOWS_E2E.md#suite-lifecycle-modes-m7); laptop walkthrough: [`systemtest/README-suite-promotion.md`](../../systemtest/README-suite-promotion.md). |
...
| 2026-05-20 | 1.8 | **M7 shipped.** `SuitePromotionService` finalised across `OFFER_AS_PR` / `PROMOTE_ON_MERGE` / `COMMIT_TO_PR` (`EPHEMERAL` remains the safe default), reusing `WorkspaceService` + `RepositoryApiClient.createPullRequest` so every provider works out of the box. Idempotency via `PrWorkflowRun.followUpPrNumber`; conflict-resolving file naming (`login.spec.ts` → `login_2.spec.ts` → `login_3.spec.ts`) so promotion never overwrites existing tests. New `PromotedSuiteGarbageCollector` (`@Scheduled` cron, default 03:17 nightly, retention via `prworkflow.e2e.promotion.retention`, default `P30D`) retires the in-DB `PrTestSuite` rows for runs whose follow-up PR has been around long enough — the promoted-PR link on `PrWorkflowRun` stays so the dashboard still surfaces "promoted as PR #N". Internal cleanup: dropped unreachable `default ->` arms in three `switch` expressions over `SuiteLifecycleMode` (compile-time safety now flags any new enum value), removed an unused `Optional` import. Operator-facing: `SUITE_PROMOTION_USER_STORY.md` flipped to ✅ shipped with new GC criterion; top-level `README.md` + `doc/DOCKERHUB_README.md` flip the "Suite promotion" row to ✅ shipped (M7). New migration guide [`doc/MIGRATION_1.6_TO_1.7.md`](../MIGRATION_1.6_TO_1.7.md) documents the additive nature of M1 – M7 (no schema breaks; all new tables + Flyway-managed). Test bar at **809 green (+11 over M6)**, including 5-test `PromotedSuiteGarbageCollectorTest`. |
